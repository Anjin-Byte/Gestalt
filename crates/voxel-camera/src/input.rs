//! The input snapshot bridging a front end's event handlers and the pure
//! [`FlyCamera`].
//!
//! The handlers (winit callbacks, DOM-event messages — the effectful edge) set
//! the held-key flags and accumulate the per-frame pointer/scroll deltas here;
//! [`FlyCamera::apply`] reads it as plain data. Held flags persist across
//! frames (they reflect key state); deltas are accumulated within a frame and
//! cleared by [`Input::end_frame`] once consumed.
//!
//! [`FlyCamera`]: crate::FlyCamera
//! [`FlyCamera::apply`]: crate::FlyCamera::apply
