...
pub fn compute_summary(
    ...
    let (aabb_min, aabb_max) = if is_empty {
        ...
        (
            [
                world_offset[0] + min_x as f32,
                world_offset[1] + min_y as f32,
                world_offset[2] + min_z as f32,
                0.0,
            ],
            [
                world_offset[0] + max_x as f32 + 1.0,
                world_offset[1] + max_y as f32 + 1.0,
                world_offset[2] + max_z as f32 + 1.0,
                0.0,
            ],
        )
    };
...
mod tests {
    ...
    fn empty_chunk() {
        ...
        let result = compute_summary(
            occ.as_words(), &pal.as_words(), &empty_materials(), [0, 0, 0],
        );
    ...
    fn full_chunk() {
        ...
        // AABB should be full chunk extent
        assert_eq!(result.aabb_min[0], 0.0);
        assert_eq!(result.aabb_min[1], 0.0);
        assert_eq!(result.aabb_min[2], 0.0);
        assert_eq!(result.aabb_max[0], 64.0);
        assert_eq!(result.aabb_max[1], 64.0);
        assert_eq!(result.aabb_max[2], 64.0);
    }
    ...
    fn single_voxel_at_17_42_31() {
        ...
        // AABB: tight around (17, 42, 31)
        assert_eq!(result.aabb_min[0], 17.0);
        assert_eq!(result.aabb_min[1], 42.0);
        assert_eq!(result.aabb_min[2], 31.0);
        assert_eq!(result.aabb_max[0], 18.0);
        assert_eq!(result.aabb_max[1], 43.0);
        assert_eq!(result.aabb_max[2], 32.0);
    }
    ...
    fn single_voxel_world_offset() {
        ...
        occ.set(10, 20, 30);
        let pal = PaletteBuilder::new();
        // Chunk at (1, 2, 3) → world offset = (62, 124, 186)
        // chunk_coord * CS per chunk-contract.md
        let result = compute_summary(
            occ.as_words(), &pal.as_words(), &empty_materials(), [1, 2, 3],
        );
        assert_eq!(result.aabb_min[0], 62.0 + 10.0);
        assert_eq!(result.aabb_min[1], 124.0 + 20.0);
        assert_eq!(result.aabb_min[2], 186.0 + 30.0);
        assert_eq!(result.aabb_max[0], 62.0 + 11.0);
        assert_eq!(result.aabb_max[1], 124.0 + 21.0);
        assert_eq!(result.aabb_max[2], 186.0 + 31.0);
    }
    ...
    fn emissive_detection() {
        ...
        let result = compute_summary(
            occ.as_words(), &pal.as_words(), &mats, [0, 0, 0],
        );
    ...
    fn sphere_aabb_containment() {
        ...
        let mats = test_materials();
        let result = compute_summary(
            chunk.occupancy.as_words(), &chunk.palette.as_words(), &mats, [0, 0, 0],
        );
            ...
            for y in 0..CS_P {
                for z in 0..CS_P {
                    if chunk.occupancy.get(x, y, z) {
                        assert!(
                            (x as f32) >= result.aabb_min[0] && (x as f32 + 1.0) <= result.aabb_max[0]
                            && (y as f32) >= result.aabb_min[1] && (y as f32 + 1.0) <= result.aabb_max[1]
                            && (z as f32) >= result.aabb_min[2] && (z as f32 + 1.0) <= result.aabb_max[2],
                            "Voxel ({x},{y},{z}) outside AABB [{:?}..{:?}]",
    ...
    fn sphere_aabb_tightness() {
        ...
        let min_x = result.aabb_min[0] as u32;
        let max_x = result.aabb_max[0] as u32 - 1;
        let min_y = result.aabb_min[1] as u32;
        let max_y = result.aabb_max[1] as u32 - 1;
        let min_z = result.aabb_min[2] as u32;
        let max_z = result.aabb_max[2] as u32 - 1;
