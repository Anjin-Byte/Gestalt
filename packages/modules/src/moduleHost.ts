import type { ModuleContext, ModuleOutput, TestbedModule, UiApi } from "./types";
...
export class ModuleHost {
  ...
  // Public so callers can wire UiApi onChange → scheduleRun externally.
  scheduleRun(): void {
    void this._scheduleRun();
  }

  private async _scheduleRun(): Promise<void> {
    if (!this.active) {
      return;
    }
  ...
  async dispose(): Promise<void> {
    ...
    for (const module of modules) {
      ...
      try {
        ...
        this.ctx.logger.warn(
          `Module ${module.id} dispose failed during dispose: ${(error as Error).message}`
        );
      }
    }
    this.active = null;
